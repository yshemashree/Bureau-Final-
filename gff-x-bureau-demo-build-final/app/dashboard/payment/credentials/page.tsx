...
import { Copy, LogIn, ShieldCheck, User, KeyRound } from 'lucide-react';
...
function CredentialsContent() {
  ...
  function copy(text: string, type: 'email' | 'password') {
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-md border border-border overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-6 py-5 border-b border-border">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">These are your login credentials</p>
                <p className="text-xs text-muted-foreground">Please login to access your account</p>
              </div>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {/* Email row */}
              <div className="flex items-center justify-between gap-3 bg-[#f5f7fa] rounded-xl px-4 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  <User size={15} className="text-muted-foreground flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground mb-0.5">Email</p>
                    <p className="text-sm font-semibold text-foreground truncate">{DEMO_EMAIL}</p>
                  </div>
                </div>
                <button
                  onClick={() => copy(DEMO_EMAIL, 'email')}
                  className="flex-shrink-0 flex items-center gap-1 text-xs text-[#003087] hover:text-[#00247b] font-medium transition-colors"
                >
                  <Copy size={12} />
              ...
              <div className="flex items-center justify-between gap-3 bg-[#f5f7fa] rounded-xl px-4 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  <KeyRound size={15} className="text-muted-foreground flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-xs text-muted-foreground mb-0.5">Password</p>
                    <p className="text-sm font-semibold text-foreground">{DEMO_PASSWORD}</p>
                  </div>
                </div>
                <button
            ...
            <div className="px-6 pb-6">
              <Button
                className="w-full bg-[#003087] hover:bg-[#00247b] text-white text-sm font-semibold py-5 rounded-xl flex items-center justify-center gap-2"
                onClick={() => router.push(`/dashboard/payment/user-login?name=${encodeURIComponent(userName)}`)}
              >
                <LogIn size={16} />
          ...
          <p className="text-center text-xs text-muted-foreground">
            Keep these credentials safe. They are used to access your PayPal account.
          </p>
        </div>
      </div>
    </div>

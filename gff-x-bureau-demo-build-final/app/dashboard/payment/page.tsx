...
import { Wallet, ShieldCheck, Lock, LogOut, Clock, CreditCard, User, ArrowRight } from 'lucide-react';
...
function PaymentContent() {
  ...
  function handlePay() {
    setPaying(true);
    setTimeout(() => {
      router.push('/dashboard/payment/review');
    }, 800);
  }
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fa] flex flex-col">
      ...
      {/* Sticky PayPal navbar */}
      <div className="sticky top-0 z-40 flex items-center justify-between px-6 py-4 border-b border-border bg-white">
      ...
      <div className="sticky top-[61px] z-30 w-full bg-[#003087] text-white px-6 py-5 shadow-md">
        <p className="text-xs font-semibold text-blue-200 uppercase tracking-widest mb-0.5">Welcome to your Account</p>
        <p className="text-2xl font-bold leading-tight">{userName}</p>
        <p className="text-sm text-blue-200 mt-1">Your identity has been verified. You can now initiate payments securely.</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 py-8">
        <div className="w-full max-w-md space-y-4">
          ...
          <div className="text-center mb-2">
            <p className="text-xs text-muted-foreground uppercase tracking-widest">Verification complete · Secure payment</p>
            <h1 className="text-2xl font-bold text-foreground mt-1">Initiate a Payment</h1>
          </div>

          {/* Single consolidated Payment Pending block */}
          <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">

            {/* Pending status bar */}
            <div className="flex items-center gap-2.5 px-5 py-3 bg-amber-50 border-b border-amber-100">
              <Clock size={14} className="text-amber-600 flex-shrink-0" />
              <span className="text-xs font-semibold text-amber-700 uppercase tracking-wide">Payment Pending</span>
              <span className="ml-auto text-xs text-amber-600">Awaiting confirmation</span>
            </div>
            ...
            <div className="p-5 space-y-5">
              ...
              {/* Amount */}
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Amount Due</p>
                ...
                <div className="flex items-center gap-1.5 mt-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">Paying to Apple Inc</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">Main Wallet</p>
                    <p className="text-xs text-muted-foreground">Available balance</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-base font-bold text-green-700">$100,000.00</p>
                  <p className="text-xs text-muted-foreground">USD</p>
                </div>
              </div>

              <div className="flex items-center gap-1.5 bg-green-50 rounded-xl px-3 py-2.5 border border-green-100">
                <ShieldCheck size={13} className="text-green-600 flex-shrink-0" />
                <p className="text-xs text-green-700">Sufficient funds available for this transaction</p>
              </div>
              ...
              <div className="flex items-center gap-2 bg-[#f5f7fa] rounded-xl px-3 py-2.5">
                ...
                <span className="text-xs text-muted-foreground">Paying as</span>
                <span className="text-xs font-semibold text-foreground ml-auto">{userName}</span>
              </div>
            </div>
          </div>
          ...
          {/* Pay button */}
          <Button
            className="w-full py-5 text-base font-semibold bg-[#003087] hover:bg-[#00247b] text-white rounded-xl flex items-center justify-center gap-2"
            onClick={handlePay}
            disabled={paying}
          >
            {paying ? 'Processing...' : (
